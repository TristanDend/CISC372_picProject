...
//Parameters: arg, which holds these following fields:
//                srcImage: The image being convoluted
//                destImage: A pointer to a  pre-allocated (including space for the pixel array) structure to receive the convoluted image.  It should be the same size as srcImage
//                algorithm: The kernel matrix to use for the convolution
//                startRow: Starting row for thread to use
//                endRow: Ending row for thread to use
//Returns: Nothing
void* convolute(void* arg){
    ThreadArg* thread_data = (ThreadArg*) arg;
    int row,pix,bit,span;
    span = thread_data->srcImage->bpp * thread_data->srcImage->bpp;

    for (row = thread_data->startRow; row < thread_data->endRow; row++) {
        for (pix = 0; pix < thread_data->srcImage->width; pix++) {
            for (bit = 0; bit < thread_data->srcImage->bpp; bit++) {
                thread_data->destImage->data[Index(pix,row,thread_data->srcImage->width,bit,thread_data->srcImage->bpp)] = getPixelValue(thread_data->srcImage,pix,row,bit,thread_data->algorithm);
            }
        }
    }
}
...
//Parameters: type: A string representation of the type
...
int main(int argc,char** argv){
    ...
    for (int i = 0; i < thread_count; i++) {
        ...
        if (i == thread_count - 1) thread_data[i].endRow = srcImage.height;
